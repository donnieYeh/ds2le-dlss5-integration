...
当前项目刻意只发布自己的代理、项目维护的 bridge carrier 和文档，不重新分发
游戏、DS2LE、ReShade、RenoDX DLSS5 addon 或 NVIDIA DLSSNR runtime；见
...
复制进当前仓库。项目 carrier 由本仓库源码维护并按已验证载体发布；第三方二进制
仍不重新分发。这样可以继续保持当前项目的第三方二进制不重新分发边界，也可以
